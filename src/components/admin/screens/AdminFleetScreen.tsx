...
export function AdminFleetScreen() {
  ...
  return (
    <AdminShell
      ...
      actions={<Button type="button" onClick={() => setShowForm((value) => !value)}><Plus className="mr-2 h-4 w-4" />{showForm ? "Close Form" : "Add Vehicle"}</Button>}
    >
        ...
        <SurfaceCard className="mt-6" title="Add vehicle" subtitle="Create a new fleet record with enough detail to render on both admin and public pages.">
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <label className="xl:col-span-3">
              <div className="mb-2 text-xs font-semibold uppercase tracking-[0.22em] text-[var(--text-soft)]">Operating cities</div>
              <div className="flex flex-wrap gap-2">{cities.map((city) => <button key={city} type="button" className={`rounded-md px-4 py-2 text-sm font-medium ${vehicleDraft.cities.includes(city) ? "bg-[var(--brand)] text-white" : "border border-[var(--border-subtle)] bg-white text-[var(--text-muted)]"}`} onClick={() => setVehicleDraft((current) => ({ ...current, cities: current.cities.includes(city) ? current.cities.filter((item) => item !== city) : [...current.cities, city] }))}>{city}</button>)}</div>
            </label>
            <label className="xl:col-span-3"><div className="mb-2 text-xs font-semibold uppercase tracking-[0.22em] text-[var(--text-soft)]">Description</div><textarea className="min-h-[120px] w-full rounded-[var(--radius-md)] border border-[var(--border-subtle)] bg-[var(--bg-surface-soft)] px-4 py-3 text-sm outline-none transition focus:border-[var(--brand)] focus:ring-4 focus:ring-[rgba(22,119,210,0.12)]" value={vehicleDraft.description} onChange={(event) => setVehicleDraft((current) => ({ ...current, description: event.target.value }))} /></label>
            <label className="xl:col-span-3"><div className="mb-2 text-xs font-semibold uppercase tracking-[0.22em] text-[var(--text-soft)]">Features</div><textarea className="min-h-[120px] w-full rounded-[var(--radius-md)] border border-[var(--border-subtle)] bg-[var(--bg-surface-soft)] px-4 py-3 text-sm outline-none transition focus:border-[var(--brand)] focus:ring-4 focus:ring-[rgba(22,119,210,0.12)]" value={vehicleDraft.featuresText} onChange={(event) => setVehicleDraft((current) => ({ ...current, featuresText: event.target.value }))} /></label>
          </div>
          <div className="mt-6 flex flex-wrap gap-3"><Button type="button" onClick={submitVehicle}>Save Vehicle</Button><Button type="button" variant="secondary" onClick={() => setShowForm(false)}>Cancel</Button></div>
        </SurfaceCard>
      )}
      ...
      <SurfaceCard className="mt-6" title="Filter fleet" subtitle="Narrow by city, status, or category.">
        <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr]">
          <label className="relative block"><Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--text-soft)]" /><input className={`${inputClassName} pl-11`} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search by model or plate" /></label>
          ...
          <article key={vehicle.id} className="group overflow-hidden rounded-[var(--radius-xl)] border border-[var(--border-subtle)] bg-white shadow-[var(--shadow-card)] transition-all duration-200 hover:-translate-y-1 hover:border-[var(--brand)] hover:shadow-[var(--shadow-card-hover)]">
            ...
            <div className="p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="font-display text-xl font-semibold tracking-[-0.04em] text-[var(--text-main)]">{vehicle.name}</div>
                  <div className="mt-1 text-sm text-[var(--text-muted)]">{vehicle.regPlate} · {vehicle.currentCity}</div>
                </div>
              </div>
              <div className="mt-4 grid gap-2 rounded-[var(--radius-lg)] bg-[var(--bg-surface-soft)] p-4 text-sm text-[var(--text-muted)] ring-1 ring-[var(--border-subtle)]">
                <div className="flex justify-between gap-4"><span>Category</span><strong className="text-[var(--text-main)]">{vehicle.category}</strong></div>
                <div className="flex justify-between gap-4"><span>Daily rate</span><strong className="font-mono text-[var(--text-main)]">{formatCurrency(vehicle.baseDailyRate)}</strong></div>
                <div className="flex justify-between gap-4"><span>Quantity</span><strong className="text-[var(--text-main)]">{vehicle.availableQuantity} / {vehicle.totalQuantity} available</strong></div>
                <div className="flex justify-between gap-4"><span>Next booking</span><strong className="text-[var(--text-main)]">{formatDateOnly(vehicle.nextBookingDate)}</strong></div>
              </div>
              <div className="mt-5 flex flex-wrap gap-2">{vehicle.cities.map((city) => <span key={city} className="rounded-full bg-[var(--brand-soft)] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.14em] text-[var(--brand-dark)]">{city}</span>)}</div>
              <div className="mt-5 grid gap-3 sm:grid-cols-[1fr_auto] sm:items-center">
                ...
                <Button asChild variant="secondary"><Link to={`/fleet/${vehicle.slug}`}>View Page<ExternalLink className="ml-2 h-4 w-4" /></Link></Button>
              </div>
            </div>
          </article>
        ))}
