...
export function AdminSettingsScreen() {
  ...
  return (
    <AdminShell
      ...
      actions={<div className="flex flex-wrap gap-3"><Button type="button" onClick={save}><Save className="mr-2 h-4 w-4" />Save Settings</Button><Button type="button" variant="secondary" onClick={reset}>Reset Defaults</Button></div>}
    >
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <SurfaceCard title="Settings areas" subtitle="Jump between operational and content controls.">
          <div className="space-y-2">{[["companyProfile", "Company profile"],["bookingRules", "Booking rules"],["pricingRules", "Pricing rules"],["notifications", "Notifications"],["paymentSettings", "Payment settings"],["cms", "Website content"]].map(([key, label]) => <button key={key} type="button" onClick={() => setActiveSection(key as keyof AdminSettings)} className={`w-full rounded-[var(--radius-md)] border px-4 py-4 text-left transition ${activeSection === key ? "border-[var(--brand)] bg-[var(--brand-soft)]" : "border-[var(--border-subtle)] bg-white"}`}><div className="font-semibold text-[var(--text-main)]">{label}</div><div className="mt-1 text-sm text-[var(--text-muted)]">Open and update this settings block.</div></button>)}</div>
        </SurfaceCard>

        <SurfaceCard title={settingsSectionTitle(activeSection)} subtitle="Changes are saved into the persistent browser store for this review environment.">
